...
class Profile_Family_New extends StatefulWidget {
  @override
  _Profile_Family_NewState createState() => _Profile_Family_NewState();
}

class _Profile_Family_NewState extends State<Profile_Family_New>
  ...
  late List familyList;
  late List dataPortalpengumuman;
  fnDataPortal(token) async {
    final response = await http.get(
        Uri.parse(BaseUrl.apiBaseUrl + 'profile/family-member'),
        headers: {HttpHeaders.authorizationHeader: "Bearer " + token});
    final result = json.decode(response.body);
    setState(() {
      familyList = result['data'];
    });
    print(familyList);
  }
                          ...
                          child: (familyList == null)
                              ...
                              : ListView.builder(
                                  ...
                                  itemBuilder:
                                    ...
                                    Map familyData = familyList[index];
                                    final int count = familyList.length > 10
                                        ? 10
                                        : hotelList.length;
                                    ...
                                    return timePanelLembur(
                                        ...
                                        familyData['SameHouse'],
                                        familyData['BirthPlace'],
                                        familyData['BirthDate'],
                                        familyData['MobilePhone'],
                                        animation);
                                  },
                                ),
                        ),
  ...
  Widget timePanelLembur(namaFamily, sex, hubungan, sameHouse, tempatLahir,
      tglLhr, no_hp, Animation<double> animation) {
    var serumah;
    if (sameHouse == true) {
      serumah = "Iya";
    } else {
